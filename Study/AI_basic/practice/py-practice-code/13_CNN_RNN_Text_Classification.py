...
def _01_LSTM_naver_sentiment_analysis():
    
    urllib.request.urlretrieve("https://raw.githubusercontent.com/e9t/" \
                                "nsmc/master/ratings_train.txt", filename="ratings_train.txt")
    urllib.request.urlretrieve("https://raw.githubusercontent.com/e9t/" \
                                "nsmc/master/ratings_test.txt", filename="ratings_test.txt")
    ...
    # ==== 데이터 전처리 및 확인 : train dataset ==== #

    print('\n훈련용 리뷰 개수 :',len(train_data)) 
    # display(train_data[:5])  # (colab)

    print('테스트용 리뷰 개수 :',len(test_data),'\n') 
    # display(test_data[:5])   # (colab)
    ...
    # display(train_data.loc[train_data.document.isnull()][:5]) # colab
    ...
    # ==== integer word encoding & decoding : wordtoidx, idxtoword==== #
    ...
    print('\n훈련 데이터의 크기 :', padded_X_train.shape)
    ...
    class TextClassifier(nn.Module):
        ...
        def forward(self, x):  # x: (batch_size, seq_length)
            ...
            # lstm_out: (batch_size, seq_length, hidden_dim)    모든시점 기록
            # hidden:   (1, batch_size, hidden_dim)             마지막 시점만 기록
    ...
    predict(test_input, model, word_to_index, index_to_tag)
# _01_LSTM_naver_sentiment_analysis()

def _02_GRU_IMDB_sentiment_analysis():
    print('\n')
    ...
    class TextClassifier(nn.Module):
        def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
            ...
            self.fc = nn.Linear(hidden_dim, output_dim) 
            # output_dim = 분류하고자하는 카테고리 개수

        def forward(self, x):
            # x: (batch_size, seq_length) == (32, 500)
            embedded = self.embedding(x)  
            # (batch_size, seq_length, embedding_dim) == (32, 500, 100) == (데이터 개수, 문장길이, 단어벡터 차원)
            
            gru_out, hidden = self.gru(embedded)  
            # gru_out: (batch_size, seq_length, hidden_dim), hidden: (1, batch_size, hidden_dim)
            # lstm 이 cell state 를 뱉는 것과는 대조적으로 hidden만 뱉음
    ...
    test_input = "This movie was just way too overrated. The fighting was not professional " \
    "and in slow motion. I was expecting more from a 200 million budget movie. The little sister of T.Challa was just trying too hard to be funny. The story was really dumb as well. Don't watch this movie if you are going because others say its great unless you are a Black Panther fan or Marvels fan."
    predict(test_input, model, word_to_index, index_to_tag)

    test_input = " I was lucky enough to be included in the group to see the advanced screening " \
    "in Melbourne on the 15th of April, 2012. And, firstly, I need to say a big thank-you to Disney and Marvel Studios. Now, the film... how can I even begin to explain how I feel about this film? It is, as the title of this review says a 'comic book triumph'. I went into the film with very, very high expectations and I was not disappointed. Seeing Joss Whedon's direction and envisioning of the film come to life on the big screen is perfect. The script is amazingly detailed and laced with sharp wit a humor. The special effects are literally mind-blowing and the action scenes are both hard-hitting and beautifully choreographed."
    predict(test_input, model, word_to_index, index_to_tag)
# _02_GRU_IMDB_sentiment_analysis()

def _03_1DCNN_IMDB_sentiment_analysis():
    nltk.download('punkt')
    nltk.download('punkt_tab')
    ...
    def predict(text, model, word_to_index, index_to_tag, min_len=5):
        model.eval()

        tokens = word_tokenize(text)
        token_indices = [word_to_index.get(token.lower(), 1) for token in tokens]
        if len(token_indices) < min_len:
            token_indices += [0] * (min_len - len(token_indices))
    ...
    test_input = "This movie was just way too overrated. The fighting was not professional and" \
    " in slow motion. I was expecting more from a 200 million budget movie. The little sister of T.Challa was just trying too hard to be funny. The story was really dumb as well. Don't watch this movie if you are going because others say its great unless you are a Black Panther fan or Marvels fan."
    print(predict(test_input, model, word_to_index, index_to_tag))

    test_input = " I was lucky enough to be included in the group to see " \
    "the advanced screening in Melbourne on the 15th of April, 2012. And, firstly, I need to say a big thank-you to Disney and Marvel Studios. Now, the film... how can I even begin to explain how I feel about this film? It is, as the title of this review says a 'comic book triumph'. I went into the film with very, very high expectations and I was not disappointed. Seeing Joss Whedon's direction and envisioning of the film come to life on the big screen is perfect. The script is amazingly detailed and laced with sharp wit a humor. The special effects are literally mind-blowing and the action scenes are both hard-hitting and beautifully choreographed."
    print(predict(test_input, model, word_to_index, index_to_tag))
# _03_1DCNN_IMDB_sentiment_analysis()

def _04_1DCNN_IMDB_sentiment_analysis_enhancement():
    nltk.download('punkt')
    nltk.download('punkt_tab')
    ...
    "in Melbourne on the 15th of April, 2012. And, firstly, I need to say a big thank-you to Disney and Marvel Studios. Now, the film... how can I even begin to explain how I feel about this film? It is, as the title of this review says a 'comic book triumph'. I went into the film with very, very high expectations and I was not disappointed. Seeing Joss Whedon's direction and envisioning of the film come to life on the big screen is perfect. The script is amazingly detailed and laced with sharp wit a humor. The special effects are literally mind-blowing and the action scenes are both hard-hitting and beautifully choreographed."
    predict(test_input, model, word_to_index, index_to_tag)
_04_1DCNN_IMDB_sentiment_analysis_enhancement()
