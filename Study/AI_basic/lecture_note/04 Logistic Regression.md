...
(정답 맞추면 오차를 부드럽게 넘기고, 틀렸을 때는 비용/벌점을 크게 부여하여 모델이 학습을 좀 더 정밀하게 할 수 있도록 도와줌. 일차함수(선형)은 모델이 매우 틀린 답. 정반대 방향의 답을 내놓아도 막대한 오차를 부여하지는 않기 떄문에 치명적인 실수를 고치려고 들지 않고, 로그함수는 이러한 문제를 해결해 줌. 더 잘 맞출수록 오차가 드라마틱하게 적어지고(일정범위에서), 더 못 맞출수록, 큰 실수를 할수록 오차가 무한대까지도 발산할 수 있기에 바로 그쪽부터 빠르게, 많이 고침(오차의 미분값=기울기가 클수록 가중치를 더 열심히 고친다는 것))
다음은 $y=0.5$ 에 대칭하는 두 개의 로그 함수 그래프입니다. ^cost
