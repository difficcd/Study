...
typedef struct {
	...
	int capacity ;
	int num ; 
	int front ; // consumer idx
	int rear ;  // producer idx
} circular_queue ;
...
void // prod (문자열을 버퍼에 저장해둠)
circular_queue_enqueue(circular_queue * buf, char * msg) 
{
	if (buf->num < buf->capacity) {
		buf->elem[buf->rear] = msg ;
		buf->rear = (buf->rear + 1) % buf->capacity ; // circular: modular
		buf->num += 1 ;
	}
}

char * // cons (버퍼에 있는 저장한 문자열 꺼내 return 하는게 cons 역할)
circular_queue_dequeue(circular_queue * buf) 
{
	char * r = 0x0 ; // dequeue할 elem 가 없으면, return NULL
	if (buf->num > 0) {
