...
int main(void)
{
	...
	for(int i=0; i<2; i++){
		workers[num].tid = num;
		workers[num].flag = 0;
		pthread_mutex_init(&workers[num].lock, NULL);
		pthread_cond_init(&workers[num].cond, NULL);
	...
	while (1) {
		...
		if (strcmp(command,"stop") == 0) {
			...
			pthread_mutex_lock(&workers[this_tid].lock) ;
			workers[this_tid].flag = 1;
			// pthread_cond_signal(&workers[this_tid].cond); 
			pthread_mutex_unlock(&workers[this_tid].lock) ;
		}
